...
export function buildReport(input: ReportInput): string {
  const { entries, attachmentsByEntry, ledger, headHash, generatedAt } = input
  ...
  const entryHtml = entries.map((entry, i) => {
    ...
    meta.push(`<dt>Recorded</dt><dd>${escapeHtml(isoLocal(entry.recordedAt))}</dd>`)
    ...
    return `
    <article class="entry" id="entry-${i + 1}">
      <header>
